...
public class Projeto {
    ...
    private String dtCriacaoProjeto;

    public String getDataCriacaoProjeto() {
        return dtCriacaoProjeto;
    }

    public void setDataCriacaoProjeto(String dataCriacaoProjeto) {
        this.dtCriacaoProjeto = dataCriacaoProjeto;
    }
    ...
    public void updateProjeto() throws SQLException{
        String sql = "update Kanbanu.Projeto set nomeProjeto = ?, descricaoProjeto = ?  where id = ? ";
    ...
    public void criarProjeto() throws SQLException {
        String sqlInsert = "insert into Kanbanu.Projeto(fk_id_usuario,dataCriacao,nomeProjeto,descricaoProjeto) values ((select id from Kanbanu.usuario where id = ?),now(), ?,?);";
    ...
    public ArrayList<String> buscarProjetoByNome() throws SQLException {
        ...
        String sql = "select nomeProjeto from Kanbanu.Projeto where fk_id_usuario = ?  ";
        PreparedStatement stm = connection.prepareStatement(sql);
        stm.setInt(1, getIDUsuario());
        stm.execute();
    ...
    public ArrayList<String> buscarProjeto() throws SQLException {
        ArrayList<String> projetos = new ArrayList<>();
        String sql = "select * from Kanbanu.Projeto where fk_id_usuario = ? and nomeProjeto = ? ";
        PreparedStatement stm = connection.prepareStatement(sql);
        stm.setInt(1, getIDUsuario());
        stm.setString(2, getNomeProjeto());
        stm.execute();
    ...
    public void deleteProjeto() throws SQLException {
        String sql = "delete from Kanbanu.Projeto where fk_id_usuario =(select id from Kanbanu.usuario where id = ?) and id = ?";
        PreparedStatement stm = connection.prepareStatement(sql);
