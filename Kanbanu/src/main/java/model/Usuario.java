...
public class Usuario {
    ...
    public void cadastrarUsuario() throws SQLException {
        String sqlInsert = "insert into Kanbanu.usuario(nomeCompleto,email,telefone,usuario,senha) values (?,?,?,?,?)";
    ...
    private ResultSet sqlConsultaUsuarioESenha() throws SQLException {
        String sql = "select * from Kanbanu.usuario where usuario = ? and senha = ?";
    ...
    public void update() throws SQLException {
        String sql = "update Kanbanu.usuario set nomeCompleto = ?, email = ?, telefone = ?, usuario = ?, senha = ?  where id = ? ";
        PreparedStatement stm = connection.prepareStatement(sql);
        stm.setString(1, this.getNomeCompleto());
        stm.setString(2, this.getEmail());
        stm.setString(3, this.getTelefone());
    ...
    public void deleteMeuUsuario() throws SQLException {
        String sql = "delete from Kanbanu.usuario where usuario = ? ";
        PreparedStatement stm = connection.prepareStatement(sql);
        stm.setString(1, this.getUsuario());
        stm.execute();
    }
    ...
    public ArrayList<Usuario> selectAll() throws SQLException {
        String sql = "select * from Kanbanu.usuario";
        PreparedStatement stm = connection.prepareStatement(sql);
        return Pesquisa(stm);
    }
    ...
    public boolean porUsuario(String usuarioCadastro) throws SQLException {
        String sql = "select * from Kanbanu.usuario where usuario = ?";
        PreparedStatement stm = connection.prepareStatement(sql);
    ...
    public boolean porEmail(String emailCadastro) throws SQLException {
        String sql = "select * from Kanbanu.usuario where email = ?";
        PreparedStatement stm = connection.prepareStatement(sql);
